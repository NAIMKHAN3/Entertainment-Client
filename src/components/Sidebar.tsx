...
const Sidebar = () => {
    const { profileImg, name, role } = useAppSelector(state => state.auth)
    return (
        <div className='border min-h-screen'>
            ...
            <h1 className='text-center text-2xl font-semibold border-b-2 pb-5 text-[#00246a]'>{role} Dashboard</h1>

            <ul className='my-5'>
                <List className='border pl-3 py-1'>My Profile</List>
                {
                    role === 'SuperAdmin' && <div>
                        <List className='border pl-3 py-1'>Add Admin</List>
